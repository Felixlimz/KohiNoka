package com.dicoding.picodiploma.kohinoka.imageprocessor
...
class Classify(assetManager: AssetManager, modelPath: String, labelPath: String, inputSize: Int) {
    ...
    fun recognizeImage(bitmap: Bitmap): List<Recognition> {
        ...
        Log.e("ERROR" , byteBuffer.toString())
    ...
    private fun getSortedResult(labelProbArray: Array<FloatArray>): List<Classify.Recognition> {
        Log.d("Classifier", "List Size:(%d, %d, %d)".format(labelProbArray.size,labelProbArray[0].size,lableList.size))

        val pq = PriorityQueue(
                MAX_RESULTS,
                Comparator<Classify.Recognition> {
        ...
        val recognitions = ArrayList<Classify.Recognition>()
